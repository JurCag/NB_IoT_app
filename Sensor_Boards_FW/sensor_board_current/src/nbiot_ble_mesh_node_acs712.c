#include "nbiot_ble_mesh_node_acs712.h"
...
{
    {
        ...
        .sensor_data.raw_value = &dcCurrentSensorData,
    }
};
...
void nbiotBleMeshNodeAcs712Main(void)
{
...
static void updateNodeData(void)
{
    ...
    dcCurrent = sensorAcs712GetCurrent();

    // NOTE: Very important to reset the net buffer beffore pushing new data,
    // otherwise it causes unpredictable behaviour.
    i = 0;
    net_buf_simple_reset(&dcCurrentSensorData);
    net_buf_simple_push_le32(&dcCurrentSensorData, *(uint32_t*)(&dcCurrent));
    ESP_LOGI(tag, "[%s] value: [%f], units [%s]", sensorNbiotSetup[i].propName, dcCurrent, sensorNbiotSetup[i].mmtUnit);
}
